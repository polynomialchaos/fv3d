...
void calc_mesh_metrics(Mesh_t *mesh)
{
    ...
    BM_MPI_ALL_REDUCE(MPIDouble, MPISum,
                      &mesh->local_volume, &mesh->global_volume);
    ...
    faces->dist_cell_1 = BM_ALLOCATE(sizeof(double) * faces->n_faces * DIM);
    faces->dist_cell_2 = BM_ALLOCATE(sizeof(double) * faces->n_faces * DIM);
    faces->internal_faces = BM_ALLOCATE(sizeof(int) * faces->n_internal_faces);
    faces->boundary_faces = BM_ALLOCATE(sizeof(int) * faces->n_boundary_faces);
    ...
    for (int i = 0; i < faces->n_faces; ++i)
    {
        ...
        for (int j = 0; j < DIM; ++j)
            faces->dist_cell_1[i * DIM + j] =
                faces->x[i * DIM + j] - cells->x[fc[0] * DIM + j];

        if (fc[1] >= 0)
        {
            for (int j = 0; j < DIM; ++j)
                faces->dist_cell_2[i * DIM + j] =
                    faces->x[i * DIM + j] - cells->x[fc[1] * DIM + j];
    ...
    BM_CHECK_EXPRESSION(k == faces->n_internal_faces);
    BM_CHECK_EXPRESSION(l == faces->n_boundary_faces);
...
Mesh_t *read_mesh_file(cstring_t mesh_file)
{
    ...
    BM_GET_HDF5_ATTRIBUTE(file_id, "dimension", HDF5Int,
                          &mesh->dimension);
    BM_GET_HDF5_ATTRIBUTE(file_id, "is_partitioned", HDF5Int,
                          &mesh->is_partitioned);

    /* the partition */
    if (is_parallel())
    {
        BM_CHECK_EXPRESSION(mesh->is_partitioned == 1);
        ...
        BM_GET_HDF5_ATTRIBUTE(group_id, "n_partitions", HDF5Int,
                              &n_partitions);
        BM_CHECK_EXPRESSION(n_partitions == get_number_of_procs());

        dims[0] = n_partitions;
        offset[0] = rank;
        count[0] = 1;
        BM_GET_HDF5_DATASET_CHUNK_N(group_id, "n_partition_cells", HDF5Int,
                                    count[0], 1, dims,
                                    offset, count, NULL, NULL,
                                    &n_partition_cells);
        BM_GET_HDF5_DATASET_CHUNK_N(group_id, "n_partition_boundaries", HDF5Int,
                                    count[0], 1, dims,
                                    offset, count, NULL, NULL,
                                    &n_partition_boundaries);
        BM_GET_HDF5_DATASET_CHUNK_N(group_id, "n_partition_faces", HDF5Int,
                                    count[0], 1, dims,
                                    offset, count, NULL, NULL,
                                    &n_partition_faces);
        BM_GET_HDF5_DATASET_CHUNK_N(group_id, "n_partition_sends", HDF5Int,
                                    count[0], 1, dims,
                                    offset, count, NULL, NULL,
                                    &n_partition_sends);
        BM_GET_HDF5_DATASET_CHUNK_N(group_id, "n_partition_receives", HDF5Int,
                                    count[0], 1, dims,
                                    offset, count, NULL, NULL,
        ...
        BM_GET_HDF5_DATASET_CHUNK_N_M(group_id, "partition_cells", HDF5Int,
                                      count, 2, dims,
                                      offset, count, NULL, NULL,
        ...
        BM_GET_HDF5_DATASET_CHUNK_N_M(group_id, "partition_boundaries", HDF5Int,
                                      count, 2, dims,
                                      offset, count, NULL, NULL,
        ...
        BM_GET_HDF5_DATASET_CHUNK_N_M(group_id, "partition_faces", HDF5Int,
                                      count, 2, dims,
                                      offset, count, NULL, NULL,
        ...
        BM_GET_HDF5_DATASET_CHUNK_N_M(group_id, "partition_sends", HDF5Int,
                                      count, 2, dims,
                                      offset, count, NULL, NULL,
                                      partition->partition_sends);
        BM_GET_HDF5_DATASET_CHUNK_N_M(group_id, "partition_sends_pid", HDF5Int,
                                      count, 2, dims,
                                      offset, count, NULL, NULL,
        ...
        BM_GET_HDF5_DATASET_CHUNK_N_M(group_id, "partition_receives", HDF5Int,
                                      count, 2, dims,
                                      offset, count, NULL, NULL,
                                      partition->partition_receives);
        BM_GET_HDF5_DATASET_CHUNK_N_M(group_id, "partition_receives_pid",
                                      HDF5Int,
                                      count, 2, dims,
                                      offset, count, NULL, NULL,
    ...
    /* the vertices */
    {
        ...
        BM_GET_HDF5_ATTRIBUTE(group_id, "n_vertices", HDF5Int, &n_vertices);

        vertices = allocate_vertices(mesh, n_vertices);

        dims[0] = vertices->n_vertices;
        dims[1] = DIM;
        BM_GET_HDF5_DATASET_N_M(group_id, "x", HDF5Double, dims, vertices->x);
    ...
    /* the cells */
    {
        ...
        BM_GET_HDF5_ATTRIBUTE(group_id, "n_cells", HDF5Int, &n_global_cells);
        BM_GET_HDF5_ATTRIBUTE(group_id, "max_cell_vertices", HDF5Int,
                              &max_cell_vertices);
        BM_GET_HDF5_ATTRIBUTE(group_id, "max_cell_faces", HDF5Int,
                              &max_cell_faces);
        ...
        BM_GET_HDF5_DATASET_SELECT_N(group_id, "id", HDF5Int,
                                     count[0], dims[0],
        ...
        dims[0] = cells->n_global_cells;
        count[0] = cells->n_local_cells;
        BM_GET_HDF5_DATASET_SELECT_N(group_id, "type", HDF5Int,
                                     count[0], dims[0],
                                     cells->stride, cells->n_local_cells,
                                     cells->type);
        ...
        dims[0] = cells->n_global_cells;
        count[0] = cells->n_local_cells;
        BM_GET_HDF5_DATASET_SELECT_N(group_id, "n_vertices", HDF5Int,
                                     count[0], dims[0],
                                     cells->stride, cells->n_local_cells,
                                     cells->n_vertices);
        ...
        BM_GET_HDF5_DATASET_SELECT_N_M(group_id, "vertices", HDF5Int,
                                       count, dims, offset[0],
                                       cells->stride, cells->n_local_cells,
                                       cells->vertices);
        ...
        count[0] = cells->n_local_cells;
        BM_GET_HDF5_DATASET_SELECT_N(group_id, "n_faces", HDF5Int,
                                     count[0], dims[0],
                                     cells->stride, cells->n_local_cells,
                                     cells->n_faces);
        ...
        BM_GET_HDF5_DATASET_SELECT_N_M(group_id, "faces", HDF5Int,
                                       count, dims, offset[0],
                                       cells->stride, cells->n_local_cells,
                                       cells->faces);
        ...
        dims[0] = cells->n_global_cells;
        count[0] = cells->n_local_cells;
        BM_GET_HDF5_DATASET_SELECT_N(group_id, "volume", HDF5Double,
                                     count[0], dims[0],
                                     cells->stride, cells->n_local_cells,
                                     cells->volume);
        ...
        count[0] = cells->n_local_cells;
        count[1] = DIM;
        BM_GET_HDF5_DATASET_SELECT_N_M(group_id, "x", HDF5Double,
                                       count, dims, offset[0],
                                       cells->stride, cells->n_local_cells,
                                       cells->x);
        ...
        count[0] = cells->n_local_cells;
        count[1] = DIM;
        BM_GET_HDF5_DATASET_SELECT_N_M(group_id, "dx", HDF5Double,
                                       count, dims, offset[0],
                                       cells->stride, cells->n_local_cells,
                                       cells->dx);
    ...
    /* the boundaries */
    {
        ...
        BM_GET_HDF5_ATTRIBUTE(group_id, "n_boundaries", HDF5Int,
                              &n_global_boundaries);
        BM_GET_HDF5_ATTRIBUTE(group_id, "max_boundary_vertices", HDF5Int,
        ...
        dims[0] = boundaries->n_global_boundaries;
        count[0] = boundaries->n_boundaries;
        BM_GET_HDF5_DATASET_SELECT_N(group_id, "face", HDF5Int,
                                     count[0], dims[0],
                                     boundaries->stride,
                                     boundaries->n_boundaries,
                                     boundaries->face);
        ...
        dims[0] = boundaries->n_global_boundaries;
        count[0] = boundaries->n_boundaries;
        BM_GET_HDF5_DATASET_SELECT_N(group_id, "distance", HDF5Double,
                                     count[0], dims[0],
                                     boundaries->stride,
                                     boundaries->n_boundaries,
                                     boundaries->distance);
        ...
        count[0] = boundaries->n_boundaries;
        count[1] = DIM;
        BM_GET_HDF5_DATASET_SELECT_N_M(group_id, "n", HDF5Double,
                                       count, dims, offset[0],
                                       boundaries->stride,
                                       boundaries->n_boundaries,
                                       boundaries->n);
        ...
        count[0] = boundaries->n_boundaries;
        count[1] = DIM;
        BM_GET_HDF5_DATASET_SELECT_N_M(group_id, "t1", HDF5Double,
                                       count, dims, offset[0],
                                       boundaries->stride,
                                       boundaries->n_boundaries,
                                       boundaries->t1);
        ...
        count[0] = boundaries->n_boundaries;
        count[1] = DIM;
        BM_GET_HDF5_DATASET_SELECT_N_M(group_id, "t2", HDF5Double,
                                       count, dims, offset[0],
                                       boundaries->stride,
                                       boundaries->n_boundaries,
                                       boundaries->t2);
    ...
    /* the faces */
    {
        ...
        BM_GET_HDF5_ATTRIBUTE(group_id, "n_faces", HDF5Int, &n_global_faces);
        BM_GET_HDF5_ATTRIBUTE(group_id, "max_face_vertices", HDF5Int,
                              &max_face_vertices);
        ...
        count[0] = faces->n_faces;
        count[1] = FACE_CELLS;
        BM_GET_HDF5_DATASET_SELECT_N_M(group_id, "cells", HDF5Int,
                                       count, dims, offset[0],
                                       faces->stride, faces->n_faces,
                                       faces->cells);
        ...
        dims[0] = faces->n_global_faces;
        count[0] = faces->n_faces;
        BM_GET_HDF5_DATASET_SELECT_N(group_id, "boundary", HDF5Int,
                                     count[0], dims[0],
                                     faces->stride, faces->n_faces,
                                     faces->boundary);
        ...
        dims[0] = faces->n_global_faces;
        count[0] = faces->n_faces;
        BM_GET_HDF5_DATASET_SELECT_N(group_id, "area", HDF5Double,
                                     count[0], dims[0],
                                     faces->stride, faces->n_faces,
                                     faces->area);
        ...
        dims[0] = faces->n_global_faces;
        count[0] = faces->n_faces;
        BM_GET_HDF5_DATASET_SELECT_N(group_id, "lambda", HDF5Double,
                                     count[0], dims[0],
                                     faces->stride, faces->n_faces,
                                     faces->lambda);
    ...
    /* the regions */
    {
        ...
        BM_GET_HDF5_ATTRIBUTE(group_id, "n_regions", HDF5Int, &n_regions);
        ...
        BM_GET_HDF5_DATASET_N(group_id, "name", HDF5String,
                              dims[0], regions->name);

        dims[0] = regions->n_regions;
        BM_GET_HDF5_DATASET_N(group_id, "is_boundary", HDF5Int,
...
void remap_local_mesh(Mesh_t *mesh)
{
    ...
    int *global_cells = BM_ALLOCATE(sizeof(int) * cells->n_global_cells);
    int *global_boundaries =
        BM_ALLOCATE(sizeof(int) * boundaries->n_global_boundaries);
    int *global_faces = BM_ALLOCATE(sizeof(int) * faces->n_global_faces);
    ...
    for (int i = 0; i < partition->n_partition_sends; ++i)
        BM_CHECK_EXPRESSION(global_cells[partition->partition_sends[i]] != -1);
    ...
    BM_DEALLOCATE(global_cells);
    BM_DEALLOCATE(global_boundaries);
    BM_DEALLOCATE(global_faces);
}

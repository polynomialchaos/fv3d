...
def calc_volume( vertices, element ):
    ...
    if element.element_type == ElementType.LINE:
        ...
        volume  = abs( np.dot( v[1] - v[0], cross_product( v[2] - v[0], v[3] - v[0] ) ) )
        volume *= 1.0/6.0
    elif element.element_type == ElementType.HEXAEDER:
        volume  = abs( np.dot( v[5] - v[4], cross_product( v[1] - v[4], v[6] - v[4] ) ) )
        volume += abs( np.dot( v[1] - v[4], cross_product( v[3] - v[4], v[6] - v[4] ) ) )
        volume += abs( np.dot( v[1] - v[4], cross_product( v[0] - v[4], v[3] - v[4] ) ) )
        volume += abs( np.dot( v[6] - v[4], cross_product( v[3] - v[4], v[7] - v[4] ) ) )
        volume += abs( np.dot( v[3] - v[1], cross_product( v[6] - v[1], v[2] - v[1] ) ) )
        volume *= 1.0/6.0
    elif element.element_type == ElementType.PRISM:
        volume  = abs( np.dot( v[1] - v[0], cross_product( v[2] - v[0], v[5] - v[0] ) ) )
        volume += abs( np.dot( v[1] - v[0], cross_product( v[5] - v[0], v[4] - v[0] ) ) )
        volume += abs( np.dot( v[4] - v[0], cross_product( v[5] - v[0], v[3] - v[0] ) ) )
        volume *= 1.0/6.0
    elif element.element_type == ElementType.PYRAMID:
        volume  = abs( np.dot( v[1] - v[0], cross_product( v[2] - v[0], v[4] - v[0] ) ) )
        volume += abs( np.dot( v[2] - v[0], cross_product( v[3] - v[0], v[4] - v[0] ) ) )
        volume *= 1.0/6.0
    else:
        raise TypeError( element.element_type )
...
def calc_distance( cells, faces, element ):
    ...
    if distance < 1e-16: raise( ValueError( 'Distance', distance ) )
    return distance

def calc_weight( cells, element ):
    if element.cell_ids[1] >= 0:
        d1 = np.dot( element.n, element.x - cells[element.cell_ids[0]].x ) / element.area
        d2 = np.dot( element.n, cells[element.cell_ids[1]].x - element.x ) / element.area

        if d1 + d2 > 1e-16: weight = d1 / (d1 + d2)
        else: raise( ValueError( 'Weight', d1 + d2 ) )
    else:
        weight = 1.0
...
def calc_normal( vertices, cells, element ):
    ...
    if angle > 90.0 and angle < 270.0: n = -n

    return n

def calc_t1( element ):
    n    = element.n
    t1   = np.array( [-n[2],0.0,n[0]] if abs( n[0] ) > abs( n[1] ) else [0.0,-n[2],n[1]] )

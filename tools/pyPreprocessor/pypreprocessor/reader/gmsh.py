    ...
    with open( file_name, 'r' ) as fp:
        while True:
            ...
            if line == '$MeshFormat':
                ...
                n_blocks, n_vertices, _, _ = [int( x ) for x in fp.readline().split( ' ' )]
                vertices = [None] * (n_vertices)

                for i in range( n_blocks ):
                    _, _, _, n_sub_blocks = [int( x ) for x in fp.readline().split( ' ' )]
                    ...
                    for j in range( n_sub_blocks ):
                        vertices[vertice_ids[j]-1] = [float( x ) for x in fp.readline().split( ' ' )]
                
                check_read_line( fp.readline(), '$EndNodes' )
            elif line == '$Elements':
                n_blocks, n_elements, _, _ = [int( x ) for x in fp.readline().split( ' ' )]
                elements = [None] * (n_elements)

                for i in range( n_blocks ):
                    id_d, id_e, e_type, n_sub_blocks = [int( x ) for x in fp.readline().split( ' ' )]

                    for j in range( n_sub_blocks ):
                        tmp = [int( x ) for x in fp.readline().strip().split( ' ' )]
                        e_id, e_nodes       = tmp[0], [x-1 for x in tmp[1:1+gmsh_elem_nodes[e_type]]]
                        entitiy             = get_entity_by_id( entities, id_d, id_e )
                        elements[e_id-1]    = Element( gmsh_to_ElementType[e_type], e_nodes, entitiy[1] )

...
import numpy as np
import h5py
from pypreprocessor.utilities import ElementType
...
def write_mesh( file_name, mesh ):
    ...
    if mesh.dimension < 2: max_cell_vertices = max_cell_vertices + 1
    if mesh.dimension < 3: max_boundary_vertices = max_boundary_vertices + 1
    if mesh.dimension < 3: max_face_vertices = max_face_vertices + 1
    ...
    with h5py.File( file_name, 'w' ) as fp:
        ...
        if mesh.is_partitioned:
            ...
            fp_g.attrs['max_partition_boundaries'] = [max( n_partition_boundaries )]
            ...
            fp_g.create_dataset( 'n_partition_boundaries', data=n_partition_boundaries )
        ...
        fp_g.create_dataset( 'xdmf', data=gen_xdmf_3d( mesh.cells, max_cell_vertices, mesh.dimension ) )
        ...
        fp_g.create_dataset( 't1', data=np.array( [x.t1 for x in mesh.boundaries] ) )
        fp_g.create_dataset( 't2', data=np.array( [x.t2 for x in mesh.boundaries] ) )
        fp_g.create_dataset( 'distance', data=np.array( [x.distance for x in mesh.boundaries] ) )

        if mesh.is_partitioned:
            fp_g.create_dataset( 'pid', data=[x.partition_id for x in mesh.boundaries] )

        fp_g.create_dataset( 'xdmf', data=gen_xdmf_2d( mesh.boundaries, max_boundary_vertices, mesh.dimension ) )
        ...
        fp_g.create_dataset( 'area', data=np.array( [x.area for x in mesh.faces] ) )
        fp_g.create_dataset( 'lambda', data=np.array( [x.weight for x in mesh.faces] ) )

        fp_g.create_dataset( 'xdmf', data=gen_xdmf_2d( mesh.faces, max_face_vertices, mesh.dimension ) )

...
################################################################################
import numpy as np
from pymeshfv3d.utilities import Mesh, ElementType, Cell, Boundary, Cell, Region, metis_part_mesh_nodal
from pymeshfv3d.utilities import calc_area, calc_distance, calc_normal, calc_t1, calc_t2, calc_volume, calc_weight
    ...
    def _element_to_cell_boundaries(self):
        ...
        for element in self.elements:
            if element.element_type.value > face_type.value:
                self.cells.append(
                    Cell(element.element_type, element.vertice_ids, element.region_id))
            else:
                self.boundaries.append(
                    Boundary(element.element_type, element.vertice_ids, element.region_id))
            ...
            try:
                for vid in face.vertice_ids:
                    for j in vertice_faces[vid]:
                        ...
                        j_face = tmp_faces[j]
                        if face.element_type != j_face.element_type:
                            continue
                        if i_sum != sum(j_face.vertice_ids):
                            continue
                        if set(face.vertice_ids) != set(j_face.vertice_ids):
                            continue
            ...
            try:
                for vid in boundary.vertice_ids:
                    for j in vertice_faces[vid]:
                        ...
                        j_face = self.faces[j]
                        if j_face.cell_ids[1] >= 0:
                            continue
                        if boundary.element_type != j_face.element_type:
                            continue
                        if i_sum != sum(j_face.vertice_ids):
                            continue
                        if set(boundary.vertice_ids) != set(j_face.vertice_ids):
                            continue

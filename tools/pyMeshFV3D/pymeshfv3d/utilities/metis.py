...
lib = cdll.LoadLibrary(os.path.join(
    lib_path, 'libmetis.{:}'.format(lib_ending)))

c_int_p = POINTER(c_int)
c_float_p = POINTER(c_float)
c_double_p = POINTER(c_double)

lib.METIS_PartMeshNodal.argtypes = [c_int_p, c_int_p, c_int_p, c_int_p,
                                    c_int_p, c_int_p, c_int_p, c_float_p, c_int_p, c_int_p, c_int_p, c_int_p]
...
def metis_part_mesh_nodal(mesh):
    ...
    i_error = lib.METIS_PartMeshNodal(byref(c_int(mesh.n_cells)), byref(c_int(mesh.n_vertices)),
                                      wrap_vector(c_int, eptr), wrap_vector(
                                          c_int, eind),
                                      wrap_vector(c_int, vwgt), wrap_vector(
                                          c_int, vsize),
                                      byref(c_int(mesh.n_partitions)), wrap_vector(
                                          c_float, tpwgt),
                                      wrap_vector(
                                          c_int, [-1] * 40), byref(c_int()),
                                      epart, npart)
